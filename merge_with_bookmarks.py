...
import fitz
from PIL import Image
# Cache for page text (by path + page index + method)
page_text_cache: dict[tuple[str,int,str], str] = {}
...
def get_page_text(path: str, page_index: int, method: str = "combined") -> str:
    """
    Return cached text for (path, page_index, method).
    method = "PDFMiner" | "PyPDF2" | "OCR" | "combined"
    """
    key = (path, page_index, method)
    ...
    if method == "PDFMiner":
        try:
            text = pdfminer_extract(path, page_numbers=[page_index], laparams=PDFMINER_LA_PARAMS) or ""
        except Exception:
            traceback.print_exc()
    elif method == "PyPDF2":
        try:
        ...
        try:
            img = pdf_page_to_image(path, page_index, dpi=200)  # keep your dpi
            text = pytesseract.image_to_string(img, lang="eng", config="--oem 3 --psm 6") or ""
        except Exception:
            traceback.print_exc()
    elif method == "combined":
        text = extract_text(path, page_index)
    ...
    for fname in files:
        ...
        if fname.lower().endswith('.pdf'):
            ...
            for i in range(total):
                ...
                try:
                    text = get_page_text(path, i, "combined")
                ...
                try:
                    extracts["PDFMiner"] = get_page_text(path, i, "PDFMiner")
                ...
                try:
                    ...
                    extracts["Tesseract"] = get_page_text(path, i, "OCR")
    ...
    if income:
        ...
        # ALWAYS append the real pages
                    for real_entry in real_entries:
                        page_text = get_page_text(real_entry[0], real_entry[1], "combined")
